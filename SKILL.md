...
- 问题是同一事项下的互斥选项（判断目标和时间范围相同）：不要拆开，按下节选项对比处理。
...
用户在同一事项下给出互斥选项（如“留在现公司还是去 B 公司”）时，领域照常选，另外传 `--question-form option_comparison`，并声明选项与取用格式。判据是判断目标与时间范围是否一致，不是句中出现几个名词；目标不同的才是独立多事项，需要请用户先选一个。

**映射必须在起卦前声明。** 看到盘面后再决定哪一爻代表哪个选项等于事后挑说法，统一入口会把映射冻结进结果，事实审计据此校验解读。

- `--mapping-mode shi_ying`：恰好两个选项。有现状项时用 `:status_quo` 标出，它落世；两个都是新动作时按声明顺序，先声明的落世。映射由脚本推导，不要传 `--bind`。
- `--mapping-mode yongshen_multi`：两个及以上选项，每个都必须用 `--bind` 显式绑定到爻位（`line:N` 或 `hidden:N`）。此时用神多现取舍挂起。

```powershell
python scripts/run_liuyao.py --question "留在现公司还是去 B 公司" --category career --method auto `
  --question-form option_comparison `
  --option "A=留在现公司:status_quo" --option "B=去 B 公司" `
  --mapping-mode shi_ying --output session.json
```

映射不完整、爻位不存在、两选项绑定同一爻位、`shi_ying` 收到 `--bind`、`travel` 领域用 `shi_ying` 时，统一入口拒绝排盘并报错，不会静默回落成单目标占问；补全参数重跑，不要自行推测。

向用户确认选项时只问选项本身，不输出取用格式和绑定爻位等内部判断。详见 [domain-routing.md](references/domain-routing.md) 的“选项对比”。
...
用户用英文提问、或明确要求英文答复时加 `--language en`。它只改变交付语言，不改变排盘：卦是同一张，事实是同一批，只是 `prompt` 会要求模型用英文写，并固定六亲、世应、纳甲、卦名的译法，好让事实审计仍然核得住。术语译法见 `scripts/lexicon.py`，那里是唯一一份表；不要在解读里另起译名。默认 `zh`，识别不了的取值也回落到 `zh`，宁可给出中文也不给出半中半英。
